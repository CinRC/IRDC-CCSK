...
public abstract class ComplexProcess extends Process {
    ...
    public LinkedList<Process> pack(LinkedList<Process> template) {
        for (int i = 0; i < template.size(); i++) {
            ...
            if (template.get(i) == this) {
                if (left == null) {
                    org.cinrc.IRDC.log(String.format("Using %s to init left ", template.get(i - 1).origin()));
    ...
    public String represent() {
        String s = "";
        if (ghostKey != null) {
            if (org.cinrc.IRDC.config.contains(RCCSFlag.SUMMATION_STYLE_1)) {
            ...
            } else if (org.cinrc.IRDC.config.contains(RCCSFlag.SUMMATION_STYLE_3)) {
                if (left.isGhost) {
                    s = represent(String.format(
                        "%s"
                        , right == null ? "" : right.represent()
    ...
    public boolean canAct(Label label) {
        org.cinrc.IRDC.log(String.format("Checking if %s can act on %s", represent(), label));
